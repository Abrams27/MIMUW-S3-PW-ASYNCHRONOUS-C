...
void nieinstant(void *args, size_t argsz __attribute__((unused))) {
  usleep(500000);
}
...
static char *all_tests() {
  ...
  mu_run_test(tasks); //to
    mu_run_test(tasks_break); //to
//    mu_run_test(wpyte_threads_no_tasks);
    mu_run_test(dokladanie); //to
//    mu_run_test(dlugietaski);
  return 0;
}
